...
export async function up(file) {
  ...
  for (let i = 0; i < blockCount; i++) {
    ...
    // 加入到 Promise 数组中
    axiosPromiseArray.push(axios.post('http://localhost:9000/file/upload', form, axiosOptions));
  }
  // 所有分片上传后，请求合并分片文件
  await axios.all(axiosPromiseArray).then(() => {
    ...
    axios.post('http://localhost:9000/file/merge_chunks', data).then(res => {
      console.log('上传成功');
